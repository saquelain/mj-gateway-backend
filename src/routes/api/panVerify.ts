...
panVerifyRouter.post('/pan-verify', authenticateApiKey, rateLimit, async (req, res) => {
  const { apiClient, apiKeyId } = req as AuthenticatedApiRequest;
  ...
  // Balance check (pre-check only — actual debit happens after provider responds)
  ...
  if (!providerResponse.success) {
    // Provider down/failed — log but charge nothing
  ...
  // Debit — only after provider responded successfully
  await debitWallet(wallet.id, price, `api_call_${logInsert.insertId}`);

  res.json({ success: true, data: providerResponse.data, cost: price });
});